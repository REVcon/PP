...
void draw_3d_model_simple(ModelPtrArray const& models, TGAImage &frame, float *zbuffer)
{
    ...
    for (auto const& pModel : models) {
		poolåTitle.runAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile1));
		poolåTitle.runAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile2));
		poolåTitle.runAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile3));
		poolåTitle.runAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile4));
